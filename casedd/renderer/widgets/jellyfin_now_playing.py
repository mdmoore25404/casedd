"""Jellyfin now-playing table widget renderer.

Consumes ``jellyfin.sessions.rows`` emitted by
:class:`~casedd.getters.jellyfin.JellyfinGetter` and renders a compact table
of active streams, styled to match the Plex now-playing table.

Row format expected (pipe-delimited, one session per line):
    USER|TITLE|MEDIA_TYPE|PROGRESS_PERCENT|DECISION

where ``DECISION`` is ``transcode`` or ``direct play``.

Public API:
    JellyfinNowPlayingWidget — :class:`~casedd.renderer.widgets.base.BaseWidget`
    subclass registered as ``jellyfin_now_playing``.
"""
...
class JellyfinNowPlayingWidget(BaseWidget):
    """Render active Jellyfin sessions in a compact table.

    Reads ``jellyfin.sessions.rows`` (or a custom ``source``) which contains
    newline-separated, pipe-delimited session rows in the format:
    ``USER|TITLE|MEDIA_TYPE|PROGRESS_PERCENT|DECISION``.
    """
    ...
    def draw(
        ...
        source = cfg.source.strip() if cfg.source else "jellyfin.sessions.rows"
        ...
        # Jellyfin brand blue as default accent
        accent = parse_color(cfg.color, fallback=(0, 164, 220))
        body_size = (
            cfg.font_size
...
def _draw_header(  # noqa: PLR0913 -- explicit column positions keep draw path fast
    ...
    """Draw now-playing table column headers."""
    color = (188, 196, 208)
...
def _decision_color(
    ...
    """Map playback decision label to a display colour."""
    mode_key = decision.strip().lower()
    if mode_key == "transcode":
        return (255, 135, 135)
    if mode_key in {"direct_stream", "direct stream"}:
        return (255, 210, 120)
    # direct play — use brand accent
    return accent


def _parse_rows(text: str) -> list[_NowPlayingRow]:
    """Parse pipe-delimited now-playing rows from a store string.

    Args:
        text: Newline-separated rows, each formatted as
            ``USER|TITLE|MEDIA_TYPE|PROGRESS_PERCENT|DECISION``.

    Returns:
        Parsed :class:`_NowPlayingRow` list; silently skips malformed lines.
    """
