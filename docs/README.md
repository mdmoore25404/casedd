...
Additional backends can be declared under `outputs:` in `casedd.yaml`. All share one data-collection pipeline — no redundant polling. The rendered frame is resized per backend when `width`/`height` differs.
...
See [Output Architecture](/output-architecture/) for full diagrams.
